...
Original file is located at
    https://colab.research.google.com/drive/1AruCdGm6qm6ZzOT1sBsTq5dvaOqL8j_m
"""

# -*- coding: utf-8 -*-
"""
Fine-tuning BERT for Prompt Injection Detection - Dataset: Qualifire Benchmark
Uses qualifire/prompt-injections-benchmark dataset with 10% holdout for golden test set.
"""

# -*- coding: utf-8 -*-
"""
Fine-tuning BERT for Prompt Injection Detection - Dataset: xxz224/prompt-injection-attack-dataset
Uses xxz224/prompt-injection-attack-dataset with 10% holdout for golden test set.
"""
...
def parse_arguments():
    ...
    class Args:
        def __init__(self):
            ...
            self.output_dir = './xxz224_model'

    return Args()

# alternative version with easy configuration
def get_config(test_mode=True, output_dir='./xxz224_model'):
...
def load_and_prepare_data(test_mode=False):
    """Load and prepare the xxz224 prompt injection attack dataset - Binary classification"""

    try:
        dataset = load_dataset("xxz224/prompt-injection-attack-dataset")
    except Exception as e:
        raise Exception(f"Failed to load xxz224 dataset: {str(e)}")
    ...
    attack_columns = [
        'naive_attack', 'escape_attack', 'ignore_attack',
    ...
    for idx, row in df.iterrows():
        # benign prompts
        if pd.notna(row['target_text']) and len(str(row['target_text']).strip()) >= 10:
            all_texts.append(str(row['target_text']).strip())
            all_labels.append(0)

        # malicious prompts
        for attack_col in attack_columns:
            if pd.notna(row[attack_col]) and len(str(row[attack_col]).strip()) >= 10:
                all_texts.append(str(row[attack_col]).strip())
                all_labels.append(1)

    print(f"Safe prompts: {all_labels.count(0)}")
    print(f"Attack prompts (all types combined): {all_labels.count(1)}")
    print(f"Total samples: {len(all_texts)}")

    if len(all_texts) == 0:
        raise Exception("No valid samples found after processing")
    ...
    if not test_mode and len(processed_df) > 5000:
        print("WARNING: Training on full dataset may exceed 4-hour Colab limit!")
        print("   Consider using test_mode=True for initial testing")

    # reserve 10% of samples for golden test set
    holdout_size = max(1, int(0.1 * len(processed_df)))  # At least 1 sample

    np.random.seed(42)
    golden_indices = np.random.choice(processed_df.index, size=holdout_size, replace=False)

    # save golden test set
    golden_test_df = processed_df.loc[golden_indices].copy()
    golden_test_df.to_csv('golden_test_xxz224.csv', index=False)
    remaining_df = processed_df.drop(golden_indices).copy()

    print(f"Golden test set (10%): {len(golden_test_df)} samples")
    print(f"Training pool (90%): {len(remaining_df)} samples")

    # for test mode, use only 10 examples (5 safe + 5 attack)
    if test_mode:
        safe_samples = remaining_df[remaining_df['label'] == 0].sample(n=min(5, len(remaining_df[remaining_df['label'] == 0])), random_state=42)
        attack_samples = remaining_df[remaining_df['label'] == 1].sample(n=min(5, len(remaining_df[remaining_df['label'] == 1])), random_state=42)
        remaining_df = pd.concat([safe_samples, attack_samples], ignore_index=True)
...
def create_visualizations(val_true, val_pred, golden_true, golden_pred, output_dir):
    ...
    plt.title('Model Performance Comparison: xxz224 Dataset (All Attack Types)')
    plt.ylabel('Score')
    plt.xlabel('Metrics')
...
def main():
    ...
    try:
        ...
        print(f"Dataset: xxz224 Prompt Injection Attack Dataset (All Attack Types)")
        ...
        print("Golden test set saved as: golden_test_xxz224.csv")
...
def check_xxz224_files():
    main_dir = './xxz224_model'
    print("Files in xxz224_model directory:")
...
def download_xxz224_complete():
    zip_filename = 'xxz224_prompt_injection_model_complete.zip'
    ...
    with zipfile.ZipFile(zip_filename, 'w', zipfile.ZIP_DEFLATED) as zipf:
        ...
        if os.path.exists('golden_test_xxz224.csv'):
            zipf.write('golden_test_xxz224.csv', 'data/golden_test_xxz224.csv')
            print("Added: golden_test_xxz224.csv")

    print(f"Package created: {zip_filename}")
    files.download(zip_filename)

check_xxz224_files()
download_xxz224_complete()
